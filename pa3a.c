 ...
 * the car's ID (process ID), entrance point (EAST or WEST), and speed
 * (in MPH).  To make sure that both cars do not collide by entering the
 * road at the same time, Car 2 waits some time before entering.  This is
 * implemented by the procedure Delay (t) which takes a time (in seconds)
 * as a parameter.  In the program, Car 2 delays by 900 seconds.  Since
 * Car 1 drives at 40 MPH, it will travel the 10-mile road in 15 minutes,
 * or 900 seconds.  Hence, Car 2 will enter at just about the time that
 * Car 1 exits.
 *
 * Exercises
 *
...
int driveRoad (int c, int from, int mph);

void Main ()
{
	if (Fork () == 0) {			/* Car 2 */
		Delay (850);			/* wait 900 simulated secs */
		driveRoad (CAR2, EAST, 60);	/* enter from West at 60 mph */
		Exit ();
	}
